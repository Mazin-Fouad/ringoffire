...
export class GameInfoComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(): void {
    if (this.card) {
      console.log('Current Card is', this.card); // card got the input currentCard
      let cardNumber = +this.card.split('_')[1]; // + to change number from string to number
      this.title = this.cardAction[cardNumber - 1].title; // -1 becacuse array starts from 0, cards start from 1
